        ...
        let file_entries = {
            if utoc_path.exists() {
                read_utoc(&utoc_path, pak_reader,pak_path)
                    ...
                    .map(|utoc_entry| {
                        FileEntry {
                            file_path: utoc_entry.file_path.clone(),
                            pak_path: PathBuf::from(pak_path),
                            pak_reader: pak_reader.clone(),
                            compressed: "N/A".to_string(),
                            uncompressed: "N/A".to_string(),
                            offset: "N/A".to_string(),
                            bulkdata: Some(utoc_entry.bulkdata_chunks),
                            package_data: Some(utoc_entry.packagedata_chunks),
                        }
                    })
                    .collect::<Vec<_>>()
            }
