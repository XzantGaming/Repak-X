...
fn get_logo_asset_for_theme(theme: &str) -> &'static str {
    match theme.to_lowercase().as_str() {
        ...
        "cyan" | "teal" | "turquoise" => "repakx_logo_cyan",
...
impl DiscordPresenceManager {
    ...
    pub fn set_theme_and_refresh(&self, theme: &str, current_state: &str, current_details: Option<&str>) -> Result<(), String> {
    ...
    pub fn set_managing_mods(&self, mod_count: usize) -> Result<(), String> {
        let state = format!("Managing {} mod{}", mod_count, if mod_count == 1 { "" } else { "s" });
