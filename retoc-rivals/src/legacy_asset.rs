...
impl FPackageNameMap {
    ...
    pub(crate) fn get(&self, name: FMinimalName) -> Cow<'_, str> {
        // Guard against invalid indices (negative or out-of-bounds) to prevent panics on malformed data
        if name.index < 0 { return "".into(); }
        let idx = name.index as usize;
        if idx >= self.names.len() { return "".into(); }

        let bare_name = &self.names[idx];
        if name.number != 0 {
...
pub(crate) fn get_package_object_full_name(package: &FLegacyPackageHeader, object_index: FPackageIndex, path_separator: char, lowercase_path: bool, package_name_override: Option<&str>) -> (String, String) {
    ...
    while !current_object_index.is_null() {
        ...
        if current_object_index.is_import() {
            ...
            if current_import_index >= package.imports.len() {
                // Broken index; stop walking to avoid panic
                break;
            }
            ...
            if current_export_index >= package.exports.len() {
                // Broken index; stop walking to avoid panic
                break;
            }
    ...
    if !package_object_outer_chain.is_empty() && package_object_outer_chain[0].is_import() {
        ...
        if package_import_index < package.imports.len() {
            let name_idx = package.imports[package_import_index].object_name;
            package_name = package.name_map.get(name_idx).to_string();
        } else {
            // Fallback: use summary name when import index is invalid
    ...
    for outer in package_object_outer_chain.get(start_object_index..).unwrap_or(&[]) {
        ...
        if outer.is_import() {
            ...
            if import_index < package.imports.len() {
                let name_idx = package.imports[import_index].object_name;
                full_object_name.push_str(&package.name_map.get(name_idx));
            } else {
                // Skip invalid import index
                continue;
            }
            ...
            if export_index < package.exports.len() {
                let name_idx = package.exports[export_index].object_name;
                full_object_name.push_str(&package.name_map.get(name_idx));
            } else {
                // Skip invalid export index
                continue;
            }
        }
    }
