    ...
    for export_index in 0..export_count {
        ...
        if object_export.first_export_dependency_index != -1 {
            ...
            for i in 0..object_export.create_before_create_dependencies {
                ...
                if preload_dependency_index < 0 { continue; }
                let pidx = preload_dependency_index as usize;
                if pidx >= builder.legacy_package.preload_dependencies.len() { continue; }
                let preload_dependency = builder.legacy_package.preload_dependencies[pidx];
    ...
    for dependant_node in &export_graph_nodes {
        if let Some(dependencies) = export_dependencies.get(&dependant_node.node) {
            for raw_dependency_node in dependencies {
                ...
                let raw_export_idx = raw_dependency_node.package_index.to_export_index() as usize;
                if raw_export_idx >= builder.zen_package.export_map.len() { continue; }
                let is_public_export = builder.zen_package.export_map[raw_export_idx].is_public_export();
